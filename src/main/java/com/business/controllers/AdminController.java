...
public class AdminController {
    ...
    public String placeOrder(@RequestParam String oName,
                             @RequestParam double oPrice,
                             @RequestParam(defaultValue = "1") int oQuantity,
                             HttpSession session,
                             Model model) {
        ...
        order.setUser(loggedUser);  // ← links user to order
    ...
    public String orderSuccess(Model model, HttpSession session) {
        ...
        Orders lastOrder = (Orders) session.getAttribute("lastOrder");

        if (lastOrder != null) {
            model.addAttribute("amount", lastOrder.getTotalAmmout());
        }

        return "Order_success"; // Thymeleaf page name
    }
